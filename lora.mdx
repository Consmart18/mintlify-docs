...
<Steps>
  <Step title="Step 1">We take the original weight matrix W and lock it. It will not change.</Step>
  <Step title="Step 2">We create two smaller matrices, A and B. If W is a 1000 X 1000 matrix (1,000,000 parameters), we might factor it down using a rank (r) of 4. Matrix A becomes 1000 X 4 and Matrix B becomes 4 X 1000. And the total parameters in A and B become 8000.</Step>
  <Step title="Step 3">We train only A and B on the new data.</Step>
  <Step title="Step 4">
    When the model runs, the output is calculated as:
    <Latex>Output = (W + A X B) X Input</Latex>
  </Step>
</Steps>

<img src="/images/LoRA.png" alt="Figure 1: Full-Parameter Fine-tuning vs LoRA Fine-tuning comparison diagram" />

<p style={{ textAlign: 'center' }}>*Figure 1: Full-Parameter Fine-tuning vs LoRA Fine-tuning ([source](https://www.geeksforgeeks.org/deep-learning/what-is-low-rank-adaptation-lora/))*</p>

In Figure 1 above, notice that the training path for full fine-tuning flows directly through the massive block of pre-trained weights. This means every parameter in the main model is being recalculated and updated. In contrast, for LoRA, the pre-trained weights are completely frozen. Instead, the training process flows exclusively through the adapter, containing the small matrices A and B.
